    ...
    public partial class ActualizarNotas : Form
    {
        ...
        private void BtnBuscar_Click(object sender, EventArgs e)
        {
            if (TxtCi.Text != "" & Validaciones.ci(TxtCi.Text) == true)
            {
                ...
                DtDefensas = ObjDefensa.SeleccionarDefensaCriterioBss(TxtNombre.Text.ToUpper(), TxtPaterno.Text.ToUpper(), TxtMaterno.Text.ToUpper(), Convert.ToInt32(TxtCi.Text), 0,1);
            }
            else
            {
                if (TxtCi.Text != "")
                ...
                //ObjDefensa.FECHA = Validaciones.ConvertirFechaSql2005_MMDDAA(DAL.Dal_Login.fecha.ToShortDateString());
                DtDefensas = ObjDefensa.SeleccionarDefensaCriterioBss(TxtNombre.Text.ToUpper(), TxtPaterno.Text.ToUpper(), TxtMaterno.Text.ToUpper(), 0, 0,1);
            }
        ...
        private void GridDefensa_CellMouseDoubleClick_1(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
        ...
        private void BtnActualizar_Click_1(object sender, EventArgs e)
        {
            ...
            if (IdDefensa > 0)
            {
                ...
                if (TxtActa.Text !="")
                {
                    ...
                    //consultamos si existe nro de acta y si se quiere modificar el nro de acta
                    if (CkbNroActa.Checked == true)
                    {
                        // validamos que el nro de acta no haya sido asignado a ninguna otra defensa
                        ...
                        //{
                            ...
                            if (BanActualizar == true)
                            {
                                ...
                                TxtActa.Clear();
                                IdDefensa = 0;
                                //actualizamos las lista de defensas
                                if (TxtCi.Text != "" & Validaciones.ci(TxtCi.Text) == true)
                                {
                                    //ObjDefensa.FECHA = Validaciones.ConvertirFechaSql2005_MMDDAA(DAL.Dal_Login.fecha.ToShortDateString());
                                    DtDefensas = ObjDefensa.SeleccionarDefensaCriterioBss(TxtNombre.Text.ToUpper(), TxtPaterno.Text.ToUpper(), TxtMaterno.Text.ToUpper(), Convert.ToInt32(TxtCi.Text), 0, 1);
                        ...
                        //else
                        //{
                        //    MessageBox.Show("El número de acta que intenta registrar para la defensa ya fue asignado a una defensa anterio, el número debe ser único", "ERROR DE DATO", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                ...
                {
                    MessageBox.Show("El campo número de acta solo debe contener numeros", "ERROR DE DATO", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    TxtActa.BackColor = Color.Red;
                }
            }
            else
        ...
        private void CkbNroActa_CheckedChanged(object sender, EventArgs e)
        {
            if (CkbNroActa.Checked == true)
            {
                if (TxtActa.Text != "")
                {
                    if (MessageBox.Show("Seguro que desea modificar el número de acta de esta defensa?, si no desea modificar deseleccione la opcion (ACTUALIZAR NUMERO DE ACTA)", "ADVERTENCIA", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        TxtActa.ReadOnly = false;
                    }
                    else
